...
class Core
{
  public function __construct()
  {
    ...
    if (file_exists($path_controller)) {
      ...
      //si la cantidad de elementos es mayoro igual a 2 es decir se ha agregado
      $size = count($url);
      if($size >= 2){
        if (method_exists($controller_name, $url[1])) {
          ...
          if ($size >= 3) {
            ...
            //el usuario no ha especificado parametros entoce llamamos a la accion sin parametros
            $controller->{$url[1]}();
          }
      ...
      //echo "El controlador {$url[0]} existe",PHP_EOL;
    } else {
      //Error 404
